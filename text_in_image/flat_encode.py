...
bits_per_pixel = 8
pixel_offset = int(number_of_pixel / (number_of_bits / bits_per_pixel))
...
for i in range(0, pixel_array.size, pixel_offset):
    ...
    if binary_string_index < len(binary_string):
        ...
        text_bits = binary_string[
            binary_string_index : binary_string_index + bits_per_pixel
        ]

        # convert value from pixel to binary with 10 bit and add text bit
        encoded_pixel = format(pixel, "08b")[:-bits_per_pixel] + text_bits
        pixel_array[i] = int(encoded_pixel, 2)
        binary_string_index += bits_per_pixel
    else:
        break
